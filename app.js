...
pulsateTimeline.to(customButton, {
  ...
  fetch("/prescriptionDrugs.json")
    .then((e) => e.json())
    .then((t) => {
      let r = document.getElementById("searchInput"),
        i = document.getElementById("customButton"),
        a = document.getElementById("resultsList");
      function n() {
        let t = e.timeline({ delay: 0.1 }),
      ...
      function o(t) {
        a.innerHTML = "";
        let r = e.timeline({ delay: 0.1 }),
          i = document.createElement("div");
        i.classList.add("row", "row-cols-1", "g-2");
        if (t.length > 1) {
          i.classList.add("row-cols-md-3");
        }
        t.forEach((e) => {
        ...
        i.addEventListener("click", () => {
          ...
          if (e) {
            let i = t.flatMap((t) =>
              t.generics.filter(
                (t) =>
                  t.brand.toLowerCase().startsWith(e) ||
                  t.name.toLowerCase().startsWith(e)
              )
            );
            0 === i.length ? n() : o(i);
          }
